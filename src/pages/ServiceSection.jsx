        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-12 max-w-4xl mx-auto">
          {services.map((item) => (
            <div key={item.id} className="flex flex-col-reverse justify-end w-full">
              ...
              {/* Title Header with Large Overlapping Gradient Ball */}
              <div className="flex items-center text-left w-full px-1 relative">
                {/* Large Blue & Green Gradient Ball overlapping the first character */}
...
<section className="w-full bg-[#e0f2fe]/50 py-16 px-4">
  <div className="max-w-7xl mx-auto text-center">
    ...
    <div className="w-full max-w-[92%] lg:max-w-[80%] bg-white rounded-2xl p-6 sm:p-10 py-12 md:py-16 shadow-sm flex flex-col md:flex-row gap-8 lg:gap-12 items-center md:items-start text-left mx-auto">
      ...
      {/* Left Column Profile Block - Remained fixed to prevent image dimensional stretching */}
      <div className="w-full max-h-[260px] max-w-[280px] sm:max-w-[320px] md:w-64 lg:w-72 shrink-0 text-center flex flex-col items-center">
        ...
        <p className="text-xs sm:text-sm md:text-xs text-gray-500 font-medium leading-relaxed tracking-wide">
          ...
          <span className="text-gray-500 block mt-1">業務統括 / 志村 恭弘 様</span>
        </p>
      </div>
      ...
      {/* ================= WORKS SECTION ================= */}
<section id="works" className="max-w-6xl mx-auto px-4 py-16 text-center">

{/* WORKS Heading */}
<h2 className="w-full max-w-5xl mx-auto mt-4">
  <svg
    viewBox="0 0 1000 130"
    className="w-full h-auto font-sans font-extrabold tracking-[0.22em]"
  >
    <defs>
...
<p className="text-sm tracking-wider text-gray-500 mt-1 mb-14">
  施工実績
</p>

{/* Works Grid */}
<div className="grid grid-cols-2 gap-x-6 gap-y-12 sm:gap-x-10 lg:gap-x-16 max-w-5xl mx-auto">
  ...
  {works.map((item) => (
    <div key={item.id} className="flex flex-col items-center">
      ...
      {/* Gradient Line */}
      <div
        className="w-[85%] h-[2px] mt-4"
        style={{
          background:
            "linear-gradient(90deg, #22a4dd 0%, #4ade80 50%, #A3E635 100%)",
        }}
      />

      {/* Title */}
      <h3 className="mt-6 text-center text-gray-600 font-semibold leading-tight whitespace-pre-line text-sm sm:text-xl">
        {item.title}
      </h3>
